...
namespace Al_shifa_clinic_lab
{
    public partial class invoice : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            printDocument1.DefaultPageSettings.PaperSize = new System.Drawing.Printing.PaperSize("pprnm",160,340);
        ...
        {
            e.Graphics.DrawString("Al_shifa Lab", new Font("Century Gothic", 12, FontStyle.Bold) , Brushes.Red, new Point(20));
            e.Graphics.DrawString(" Name: "+tbPatName.Text, new Font("Century Gothic", 9, FontStyle.Bold), Brushes.Black, new Point(15,19));
            e.Graphics.DrawString(" Test            Cost", new Font("Century Gothic", 10, FontStyle.Bold), Brushes.Red, new Point(15,40));
            foreach (DataGridViewRow row in dgvTestList.Rows)
            {
                ...
                e.Graphics.DrawString("" + TestName, new Font("Century Gothic", 9, FontStyle.Bold), Brushes.Blue, new Point(-2, pos));
                e.Graphics.DrawString("" + TestCost, new Font("Century Gothic", 9, FontStyle.Bold), Brushes.Blue, new Point(96,pos));
                pos = pos + 20;
            }
            e.Graphics.DrawString("Total: RS " + total, new Font("Century Gothic", 12, FontStyle.Bold), Brushes.Crimson, new Point(15, pos+10));
        ...
        public void show()
        {
            try
            {
                ...
                queryClass.selectds("Select Total from invoice where Total NOT IN(0)");
                int income = 0;
                for (int j = 1; j < queryClass.ds.Tables[0].Rows.Count; j++)
                {
                    DataRow rw = queryClass.ds.Tables[0].Rows[j];
                    income = income + rw.Field<Int32>(0);

                }
                Properties.Settings.Default.Income = income.ToString();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (iExit == DialogResult.Yes)
                    {
                        string qry = string.Format("delete from invoice where Invoice_id=@i");
